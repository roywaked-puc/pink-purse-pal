...
BEGIN
  ...
  -- Nada a dividir se não há recebimento a partir do início do controle
  SELECT EXISTS (
    SELECT 1 FROM public.transactions
     WHERE appointment_id = p_appointment_id
       AND type = 'entrada'
       AND is_caixa_reserva_split = false
       AND (v_inicio IS NULL OR date >= v_inicio)
  ) INTO v_any;
  ...
  -- 4. Divisão acumulada por agendamento (apenas recebimentos a partir do início)
